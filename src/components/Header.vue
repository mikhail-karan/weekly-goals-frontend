<template>
  <div
    class="flex flex-row container mx-auto text-white h-1/6 xs:h-1/5 xs:mb-6"
  >
    <div
    ...
    >
      <router-link to="/" class="flex items-center h-full">
        ...
        <h1 class="text-5xl xs:text-4xl cursor-pointer select-none">
          Weekly Growth Goals
        </h1>
      </router-link>
      ...
      <h2 v-if="auth" class="text-2xl flex flex-row">
        ...
        ><a class="group cursor-pointer"
          ><span class="hover:underline hover:text-purple-600 select-none"
            >{{ user.username }}
          </span>
          <transition
          ...
          >
            <div
              class="hidden group-hover:flex absolute z-10 p-3 bg-cardColor shadow-md justify-cente rounded-md text-lg"
            >
              <ul class="list-none">
                <li class="hover:underline select-none hover:text-purple-600">
                  <router-link to="/mygoals">my goals</router-link>
                </li>
                <li
                  class="hover:underline select-none hover:text-purple-600"
                  @click="logout"
                >
                  logout
                </li>
                <li
                  class="hover:underline text-red-500 select-none hover:text-red-600"
      ...
      <div v-else class="flex flex-row items-end">
        <h2 class="text-2xl text-purple-400 flex flex-row mr-3">
          <span
            @click="$router.push('/register')"
            class="hover:underline cursor-pointer hover:text-purple-600"
            >register</span
          >
        </h2>
